<?php 
    ...
    class Post_Illumination extends Scyllar {
        ...
        public function show_posts($post_type,$videoPath,$photoPath, $group_identity){
            $select_posts = "SELECT * FROM user_group_post WHERE post_type='$post_type' AND group_identity='$group_identity' ORDER BY created_on DESC";
        ...
        public function show_comments($email, $post_id,$getPost_type){
            $select_comments = "SELECT * FROM user_group_post_comment WHERE user_email='$email' AND post_identity='$post_id'";
                ...
                $fetch_my_image = mysqli_fetch_assoc($execute_my_image);
                    ...
                    <div class="each-comment">
                        <div class="inner-commet">
                            <div><div class="commeter-img"><img src="<?php echo '../Images/profile-img/profile-image/'.$profile_image; ?>" alt="" width="100%" height="100%"></div></div>
                            <div class="comment">
                                <div class="commeter-name"><?php echo $getFirstname; ?> <?php echo $getLastname; ?></div>
                                <div class="real-comment"><span><?php echo $getComment; ?></span></div>
                                <div class="user-accurancy">
                                    <div class="comment-count" id="<?php echo $getComment_identity; ?>" data-target="#comment-found-<?php echo $getComment_identity; ?>" data-toggle="collapse" onclick="comment_reply_display(this)">
                                        <i class="fa fa-comment-o mr-1"></i> 
                                        <span class="pre-add-comment"> Comments </span> <span id="count_comment_comments_found_<?php echo $getComment_identity; ?>">
                                            <script>
                                                setInterval(() => {
                                                    ...
                                                    $(document).ready(function(){
                                                        $("#count_comment_comments_found_<?php echo $getComment_identity; ?>").load("group_posts.box.php",{
                                    ...
                                    <div class="likes" id="<?php echo $getComment_identity; ?>" onclick="like_dislike_comment(this)">
                                        <i class="fa fa-heart-o mr-1" id="<?php echo $getComment_identity; ?>"></i> 
                                        <span class="pre-add-comment"> Likes </span> <span id="count_likes_comments_comments_<?php echo $getComment_identity; ?>">
                                            <script>
                                                setInterval(() => {
                                                    $(document).ready(function(){
                                                        ...
                                                        $("#count_likes_comments_comments_<?php echo $getComment_identity; ?>").load("group_posts.box.php",{
    ...
    if(isset($_POST['submit_file'])){
        ...
        if($_FILES['file_media']['name'] == ""){
            ...
            $newFames = new Post_Illumination($user_poster_email);
            if($newFames->save_post($empty, $imageFileType, $caption_inputed, $email_inputted, $media_section_type, $Get_group_name, $getGroup_identity)){
                // header("location: Group_admin.php?group={$url_code}&msg=success");
                // exit();
                echo "successfully posted";
            }else{
                // header("location: Group_admin.php?group={$url_code}&msg=fail");
                // exit();
                echo "fail to save";
            }
        }else{
            ...
            if(!in_array(strtolower($imageFileType), $valid_extensions)) {
                // header("location: Group_admin.php?group={$url_code}&msg=extentension");
                // exit();
                echo "check this file extension";
            }else{
                if($file_size < 20971520){
                    if(move_uploaded_file($_FILES['file_media']['tmp_name'],$location)){
                        ...
                        // header("location: Group_admin.php?group={$url_code}&msg=move_fail");
                        // exit();
                        echo "move uploaded file fail, try another";
                    }
                }else{
                    // header("location: Group_admin.php?group={$url_code}&msg=size");
                    // exit();
                    echo "this file'size is too large";
                }
            }
        }
    }
    ...
    if(isset($_POST['getPost_email'])){
        ...
        $newFames->show_posts($_POST['getpost_type'],$_POST['getPhotoPath'], $_POST['getPhotoPath'], $_POST['group_identity']);
    }
    ...
    if(isset($_POST['post_count_comment'])){
        ...
        $newCount->countComment($_POST['post_count_comment']);
    }
