<?php 
    ...
    class Post_Illumination extends Scyllar {
        ...
        // =====================================================================================================
        public function show_posts($videoPath,$photoPath){
            ...
            if(mysqli_num_rows($execute_posts) > 0){
                ...
                <div class="no-data-found d-flex flex-column text-center">
                    <img src="../Images/Illustrator_img/post_online_dkuk.png" alt="" width="100%">
        ...
        public function add_comment($comment,$post_idea){
            ...
            if($like_comment == "Off"){
                ...
                $insert_noti_setting = "INSERT INTO notifications VALUE ('','$this->E_mail','$poster_email', 'nolink', '$notification','comment','0','new','on','$create_on')";
                            ...
                            <div class="answerer-detail ml-2">
                                ...
                                <div class="accurancy d-flex justify-content-between">
                                    ...
                                    <div class="question-likes" id="<?php echo $getComment_identity; ?>" onclick="like_dislike_comment(this)">
                                        <?php 
                                            $select_like = "SELECT * FROM user_common_comment_like WHERE email='$this->E_mail' AND comment_identity='$getComment_identity' AND status_like='1'";
                                            $execute_like = mysqli_query($this->Frequency(), $select_like);
                                            if(mysqli_num_rows($execute_like) > 0 ){ ?>
                                                <i class="fa fa-heart" style="color: #14599d;" id="<?php echo $getComment_identity; ?>"></i>
                                            <?php }else{ ?>
                                                <i class="fa fa-heart-o" id="<?php echo $getComment_identity; ?>"></i>
                                            <?php } ?> Likes <span id="count_comment_found_<?php echo $getComment_identity; ?>">
                                        <script>
                                            setInterval(()=> {
        ...
        public function like_dislike_post($post_id){
            ...
            if(mysqli_num_rows($execute_check) > 0 ){
                ...
                $get_post_type = "SELECT * FROM user_common_post WHERE identity='$post_id'";
                $execute_type = mysqli_query($this->Frequency(), $get_post_type);
                $fetch_type = mysqli_fetch_assoc($execute_type);
                    $post_type = $fetch_type['post_type'];

                $insert_new_like = "INSERT INTO user_common_post_likes VALUE ('','$getIdentity','$this->E_mail','$post_id','$post_type','1','$created_on')";
                $execute_saving = mysqli_query($this->Frequency(), $insert_new_like);
                ...
                if($like_comment == "Off"){
                    ...
                    $create_on = Date("Y-m-d h:m:s");
                    $insert_noti_setting = "INSERT INTO notifications VALUE ('','$this->E_mail','$poster_email', 'nolink', '$notification','like','0','new','on','$create_on')";
                    $execute_new_noti = mysqli_query($this->Frequency, $insert_noti_setting);
                } 
                if($execute_saving){
                    return true;
    ...
    if(isset($_POST['getPost_email'])){
        ...
        $newFames->show_posts($_POST['getVideoPath'], $_POST['getPhotoPath']);
    }
    ...
    if(isset($_POST['getComment_count_like_identity'])){
        ...
        exit(json_encode($newCount_comment->count_comment_likes($_POST['getComment_count_like_identity'])));
    }
